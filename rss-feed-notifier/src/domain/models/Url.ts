...
export class Url {
   ...
   * ホスト名とパス名を取得する
   *
   * @returns ホスト名とパス名（例: "example.com/path"）
   */
  getHostWithPath(): string {
    try {
      const parsed = new URL(this.value);
  ...
  /**
   * URLがプライベートIPアドレスかどうかチェックする
   *
   * @returns プライベートIPアドレスの場合true
   */
  isPrivateIp(): boolean {
    const hostname = this.getHostname();
    return /^(10|172\.16|192\.168)\./.test(hostname);
  }
